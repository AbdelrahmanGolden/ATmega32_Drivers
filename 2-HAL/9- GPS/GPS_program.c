...
volatile u8 GPS_Buffer[GPS_BUFFER_SIZE];
 ...
 */
u8 GPS_voidUpdate(void)
{
    u16 len = UART_Receive_String(GPS_Buffer, GPS_BUFFER_SIZE);
    ...
    if (GPS_PARSE_GPGGA && strncmp((char*)GPS_Buffer, "$GPGGA,", 7) == 0)
    {
        ...
        // Parse fix quality
        GPS_Data.fixQuality = (u8)atoi(fields[GPGGA_IDX_FIX_QUALITY]);
        if (GPS_Data.fixQuality == 0) return 0; // no fix
        // Satellites
        GPS_Data.numSatellites = (u8)atoi(fields[GPGGA_IDX_NUM_SATS]);
        // Latitude
        {
    ...
    else if (GPS_PARSE_GPRMC && strncmp((char*)GPS_Buffer, "$GPRMC,", 7) == 0)
    {
        ...
        // UTC time
        {
            ...
            u8 hh = (t[0]-'0')*10 + (t[1]-'0');
            u8 mm = (t[2]-'0')*10 + (t[3]-'0');
            u8 ss = (t[4]-'0')*10 + (t[5]-'0');
...
/** Getters **/
u8 GPS_u8GetSatelliteCount(void) { return GPS_Data.numSatellites; }
...
u8 GPS_u8GetUTCTime(u8 *h,u8 *m,u8 *s)
{
    if (!h||!m||!s) return 0;
    *h = GPS_Data.utcHours;
